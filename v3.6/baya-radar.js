...
(function (global) {
  ...
  // like cleanRow, but for raw (pre-normalization) percentages, which routinely
  // exceed the radar's maxValue (e.g. 211.4) — so no upper clamp is applied.
  function cleanRawRow(row, len) {
    var out = (row || []).slice(0, len);
    while (out.length < len) out.push(null);
    return out.map(function (v) {
      if (v === null || v === undefined || v === "") return null;
    ...
    if (Array.isArray(cfg.products) && cfg.products.length) {
      cfg.products = cfg.products.map(function (p, i) {
        return {
          ...
          rawValues: cfg.series.map(function (_, si) {
            return cleanRawRow(p.rawValues && p.rawValues[si], cfg.axes.length);
          })
        };
      });
    } else {
  ...
  function create(target, userConfig) {
    ...
    // raw (pre-normalization) counterparts of seriesValues/matchedBayaAvg — used
    // only by the benefit panel/overall card/drill-down below, so the point
    // gap they show (e.g. "119.5") matches the source benchmark percentages
    // directly instead of a ratio of the radar's normalized 0-100 scores.
    function rawSeriesValues(si) {
      if (MULTI) {
        if (productSel != null) return cfg.products[productSel].rawValues[si];
        return cfg.axes.map(function (_, ai) {
          var vals = cfg.products.map(function (p) { return p.rawValues[si][ai]; })
            .filter(function (v) { return v != null; });
          return vals.length
            ? vals.reduce(function (a, b) { return a + b; }, 0) / vals.length : null;
        });
      }
      return (cfg.series[si].rawValues || cfg.series[si].values);
    }

    function matchedBayaRawAvg(si, ai) {
      if (!MULTI) return rawSeriesValues(0)[ai];
      if (productSel != null) return cfg.products[productSel].rawValues[0][ai];
      var sum = 0, count = 0;
      cfg.products.forEach(function (p) {
        if (p.rawValues[si][ai] == null) return;
        var b = p.rawValues[0][ai];
        if (b == null) return;
        sum += b; count++;
      });
      return count ? sum / count : null;
    ...
    // data for it (nulls excluded), as a raw point difference (Baya's raw % minus
    // the alternative's raw %, e.g. 183.0 - 63.5 = 119.5) — not a ratio of the
    // radar's normalized scores. Uses the current context: the isolated design,
    // or averages across all designs.
    function benefitRows() {
      ...
      cfg.axes.forEach(function (axis, ai) {
        ...
        for (var si = 1; si < cfg.series.length; si++) {
          ...
          var v = rawSeriesValues(si)[ai];
          if (v == null) continue;
          var b = matchedBayaRawAvg(si, ai);
          if (b == null) continue;
    ...
    // same per-metric point differences (benefitRows()), not a separately-computed
    ...
    function renderMetricView(ai) {
      ...
      if (visible[0]) {
        cfg.series.forEach(function (s, si) {
          ...
          var v = rawSeriesValues(si)[ai];
          if (v == null) return;
          var baya = matchedBayaRawAvg(si, ai);
          if (baya == null) return;
          rows.push({
      ...
      if (!rows.length) {
        ...
        // headline: "Baya leads by 120 points" (vs the best alternative)
        var best = rows.reduce(function (a, b) { return b.pct < a.pct ? b : a; });
        var headPct = Math.round(best.pct * 10) / 10;
        var head = headPct > 0
          ? "Baya leads the best alternative by " + headPct.toFixed(1) + " points on " + ax.label + "."
          : headPct < 0
            ? "Baya trails the best alternative by " + Math.abs(headPct).toFixed(1) + " points on " + ax.label + "."
            : "Baya is on par with the best alternative on " + ax.label + ".";
