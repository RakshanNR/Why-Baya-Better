...
(function (global) {
  ...
  function create(target, userConfig) {
    ...
    function renderBenefit() {
      ...
      renderBenefitRows(rows.map(function (r) {
        return {
          ...
          vs: "vs " + r.vs,
          pct: r.pct,
          title: "Show " + r.label + " side by side",
    ...
    function updateOverall() {
      ...
      // the average of exactly the per-metric benefits shown in the rows above,
      // so the card always agrees with the numbers on screen
      var rows = selected == null ? benefitRows() : [];
      if (!rows.length) {
        refs.overall.style.display = "none";
        return;
      }
      refs.overall.style.display = "";
      // average the ROUNDED row values, so checking the visible numbers adds up exactly
      var pct = rows.reduce(function (a, r) { return a + Math.round(r.pct); }, 0) / rows.length;
